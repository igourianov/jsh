...
Stop and report, writing no file, when either holds:

- The posting is outside the candidate's discipline.
- The company or the recruiter behind the posting is blacklisted. Report the entry and its reason.
...
4. If overwriting, carry the existing **Progress** log into the new file unchanged and append a new `Saved` entry dated today. Never discard log entries: a rescreen is a new cycle of the same application. **Status** is derived, so it needs no preserving.

## Step 4: Parse

Extract the following from the job posting:

- **Title:** what the company calls the role. It says nothing reliable about the role's actual level and is never used for scoring; its one job downstream is picking the header of the tailored resume. Capture two forms:
  - Original: as-is from the posting.
  - Normalized: stripped of team/product mentions, tech stack details and domain qualifiers, seniority wording kept as written. "Senior Engineering Manager, Platform" → "Senior Engineering Manager".
...
**Categories:** use the category vocabulary. If a qualification fits none of them, create a new category with an appropriate name. If it fits more than one, split it between those categories instead of picking the best one.

## Step 7: Classify Against the Baseline
...
**The whitelist is closed.** Match against what is written or mark it `scored`. Never reason about whether a requirement is "typical" for the role, and never treat an entry's absence as an oversight. That judgment is what the whitelist exists to remove: it is the only thing keeping scores comparable between screens.

**It carries no weights.** It contains no numbers except hard bounds. Do not infer importance, priority or weighting from it in any way.
...
**Under-scoped asks are not qualifications.** A posting below a grade's `below:` bound is a fact about the job's seniority, not a gap. Do not emit it. Carry it to Step 10 for the take.
...
**Score it (run script).** Following the Temp Files convention in the project CLAUDE.md, write the full qualification set from Step 7 to a temp file, with `match_value` added to every scored item, and run:
```
...
`tier` is required on every entry. Baseline entries carry no `weight` and no `match_value`; the script rejects the input if one does. Weights are relative, so use the Step 6 numbers as written: the script normalizes them to sum to 100 **across scored qualifications only**, which is what gives the job-specific factors their full weight.

The script outputs three things:
- the final match percentage
- `Thin: true` when there are fewer than four scored qualifications, so one unusual requirement can swing the result. Carry it to Step 10
...
**Note the engineering domain's own match value.** Non-Product postings spend scored weight on domain fit that Product postings never spend, so they sit lower on a match-sorted list even when they are good fits. Carry that number to Step 10, where the one-line take has to account for it.
...
An employment type the candidate does not accept. Do not flag a type they target or will entertain.
...
Step 8 gives direct evidence: a posting that produced few or no scored qualifications is boilerplate by measurement, not by impression. A `Thin: true` result with nothing distinctive in it is this flag.
...
Total compensation below the candidate's floor, or significantly below market rate for the role, level and location. Compare the posting's total against the floor: base plus bonus and equity, valuing any component the posting mentions without a number at its assumed rate.
Also flag pay structured in a form the candidate does not target, e.g. hourly, while the posting presents the role as full-time. An openly contract role goes under Employment type mismatch instead.

### Location mismatch
Role requires on-site or hybrid presence somewhere the candidate cannot work from or will not commute to. Remote roles are never flagged. Do not flag if the posting explicitly allows remote work.
...
**Qualification sections.** Paste the `## Qualifications` block from the Step 8 script output as-is. Do not reorder it, renumber it, re-weight it or drop items from it: it already carries the category headings and the per-item annotations in final form.
...
## Step 11: Validate and reconcile
