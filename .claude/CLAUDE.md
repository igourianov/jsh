...
- Ghost sweep is a query, not a judgment call: `node scripts/job.mjs ghost` lists anything silent past 21 days, `--apply` marks them. `--once` is for scheduled callers: it sweeps at most once a day and is the only mode that records the run, in the gitignored `.job-state.json`. A `SessionStart` hook runs `ghost --apply --once` at the start of every session. A sweep you run by hand always executes and leaves that date alone.
