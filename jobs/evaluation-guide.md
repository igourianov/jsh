# Job Application Evaluation Guide

This guide defines the methodology for comparing Ilia's resume against job postings.

## Output Format

When evaluating a job posting, always provide:

- **Match:** [Percentage]%
- **Coding:** [Percentage]% (as required by the role)
- **Salary:** [Range] (take from job posting, or estimate if not available - explicitly state if it's an estimate)
- **Gaps:** [bullet points]
...
### 1. Hands-on Technical Work
**(Assumed for all EM roles, do NOT include in output)**
...
**(Estimate and include in output)**

Estimate what % of the **JOB** requires writing production code (not the candidate's abilities):
